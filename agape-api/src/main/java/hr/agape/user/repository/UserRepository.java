...
public class UserRepository implements PanacheRepository<UserEntity> {
    ...
    public long countDirectoryExcludingUser(String q, Long excludedUserId) {
        if (excludedUserId == null) excludedUserId = -1L;

        if (q == null || q.isBlank()) {
            return count("id <> ?1", excludedUserId);
        }
    ...
    public List<UserEntity> pageDirectoryExcludingUser(String q, Long excludedUserId, int page, int size) {
        if (excludedUserId == null) excludedUserId = -1L;

        int offset = page * size;

        if (q == null || q.isBlank()) {
            return find("id <> ?1 ORDER BY LOWER(username) ASC, id ASC", excludedUserId)
                    .range(offset, offset + size - 1)
                    .list();
        }
