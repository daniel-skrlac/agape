...
public class ItemDirectoryRepository {
    ...
    public List<ItemDirectoryView> findItemsByCodes(
        ...
        List<String> safeNumericCodes = numericCodes == null || numericCodes.isEmpty()
                ? List.of("-1")
                : numericCodes;

        List<String> safePaddedNumericCodes = paddedNumericCodes == null || paddedNumericCodes.isEmpty()
        ...
        String numericPlaceholders = safeNumericCodes.stream()
                .map(x -> "?")
                .collect(Collectors.joining(","));

        String paddedPlaceholders = safePaddedNumericCodes.stream()
                .map(x -> "?")
                .collect(Collectors.joining(","));
                    ...
                    SELECT
                     ...
                     WHERE g.SKLADISTE_ID = ?
                       ...
                       AND (
                            ...
                            OR UPPER(TRIM(z.ARTIKLID)) IN (%s)
                            OR TO_CHAR(g.ARTIKL_ID) IN (%s)
                            OR LPAD(TO_CHAR(g.ARTIKL_ID), 4, '0') IN (%s)
                       )
                )
                SELECT ITEM_ID, CODE, NAME, UNIT, BARCODE
                  FROM base
        ...
        return jdbc.query(
                ...
                ps -> {
                    ...
                    for (String code : safePaddedNumericCodes) {
                        ps.setString(i++, code);
                    }

                    for (String code : safeNumericCodes) {
                        ps.setString(i++, code);
                    }
